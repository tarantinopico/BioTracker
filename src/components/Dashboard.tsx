...
export default function Dashboard({ 
  ...
  const activeEffects = useMemo(() => {
    const effectsMap: Record<string, { intensity: number; valence: string }> = {};
    
    activeSubstanceDetails.forEach(({ substance, level }) => {
      substance!.effects?.forEach(effect => {
        const intensity = (level * Math.abs(effect.intensity)) / 10;
        if (!effectsMap[effect.type]) {
          effectsMap[effect.type] = { intensity: 0, valence: effect.valence };
        }
        effectsMap[effect.type].intensity += intensity;
      });
    });
    ...
    return Object.entries(effectsMap)
      .map(([type, data]) => ({ 
        type, 
        intensity: Math.min(data.intensity, 100),
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <section className="bg-white/[0.03] backdrop-blur-2xl rounded-[32px] p-6 border border-white/[0.05] relative overflow-hidden shadow-2xl">
        <div className="absolute top-0 right-0 w-48 h-48 bg-cyan-primary/10 blur-[80px] rounded-full -mr-24 -mt-24" />
        <div className="absolute bottom-0 left-0 w-32 h-32 bg-purple-500/5 blur-[60px] rounded-full -ml-16 -mb-16" />
        
        <div className="flex items-center justify-between mb-8 relative z-10">
          <div className="flex items-center gap-4">
            <div className={cn("p-4 rounded-2xl backdrop-blur-md border border-white/5", systemLoad.bg)}>
              <systemLoad.icon className={cn("w-7 h-7", systemLoad.color)} />
            </div>
            <div>
              <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Systémový Status</h2>
              <div className={cn("text-xl font-black tracking-tight", systemLoad.color, systemLoad.pulse && "animate-pulse")}>
                {systemLoad.label}
              </div>
            </div>
          </div>
          <div className="text-right">
            <div className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Čas k detoxu</div>
            <div className="text-2xl font-black text-white cyan-text-glow">
        ...
        <div className="space-y-5 relative z-10">
          {activeSubstanceDetails.length > 0 ? (
              ...
              >
                <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest">
                  <div className="flex items-center gap-3">
                    ...
                    {tolerance > 10 && (
                      <span className="text-[8px] px-2 py-0.5 rounded-full bg-white/5 text-slate-500 border border-white/5">
                        TOL: {tolerance.toFixed(0)}%
                      </span>
                    )}
                  </div>
                  <span style={{ color: substance!.color }}>{level.toFixed(1)}%</span>
                </div>
                <div className="h-2 bg-white/[0.02] rounded-full overflow-hidden border border-white/[0.05]">
                  <motion.div 
                    initial={{ width: 0 }}
                    animate={{ width: `${level}%` }}
            ...
            <div className="py-8 text-center border border-dashed border-white/10 rounded-3xl bg-white/[0.01]">
              <span className="text-xs text-slate-600 font-bold uppercase tracking-[0.3em]">Všechny systémy čisté</span>
            </div>
          )}
        </div>
          ...
          <div className="mt-8 space-y-3 relative z-10">
            {interactions.map((alert, i) => (
              <motion.div 
                ...
                className={cn(
                  "p-4 rounded-2xl border backdrop-blur-md flex items-start gap-4",
                  alert.severity === 'high' ? "bg-red-500/5 border-red-500/20" : "bg-amber-500/5 border-amber-500/20"
                )}
              >
                <div className={cn("p-2 rounded-xl", alert.severity === 'high' ? "bg-red-500/10" : "bg-amber-500/10")}>
                  <AlertCircle className={cn("w-4 h-4", alert.severity === 'high' ? "text-red-500" : "text-amber-500")} />
                </div>
                <div>
                  <div className={cn("text-[10px] font-black uppercase tracking-widest", alert.severity === 'high' ? "text-red-500" : "text-amber-500")}>
                    {alert.type}
                  </div>
                  <div className="text-[11px] text-slate-400 leading-relaxed mt-1 font-medium">
                    {alert.message}
                  </div>
                </div>
              </motion.div>
      ...
      <section className="bg-white/[0.03] backdrop-blur-2xl rounded-[32px] p-6 border border-white/[0.05] shadow-xl">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-2">
            <Sparkles size={12} className="text-amber-400" /> Aktuální Účinky
          </h2>
        ...
        <div className="grid grid-cols-1 gap-4">
          {activeEffects.length > 0 ? (
            activeEffects.map((effect) => {
              ...
              return (
                <div key={effect.type} className="flex items-center gap-4 group">
                  <div className={cn(
                    "w-10 h-10 rounded-2xl flex items-center justify-center border border-white/5 transition-transform group-hover:scale-110",
                  ...
                  <div className="flex-1 space-y-1.5">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">{effect.type}</span>
                      <span className="text-[10px] font-black text-white">{effect.intensity.toFixed(0)}%</span>
                    </div>
            ...
            <div className="py-6 text-center bg-white/[0.01] rounded-2xl border border-dashed border-white/5">
              <span className="text-[10px] text-slate-600 font-bold uppercase tracking-widest">Žádné znatelné účinky</span>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="bg-white/[0.03] backdrop-blur-2xl rounded-[32px] p-6 border border-white/[0.05] shadow-xl">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-2 items-center">
            <div className={cn("w-2 h-2 rounded-full animate-pulse", chartType === 'kinetic' ? "bg-cyan-primary" : "bg-amber-400")} />
            <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Live Analysis</span>
          </div>
        </div>
        ...
        <div className="flex justify-between mt-6 px-4">
          <div className="flex flex-col items-center">
            <span className="text-[9px] font-black text-slate-600 uppercase tracking-tighter">Minulost</span>
            <span className="text-[8px] font-bold text-slate-700">-{settings.chartWindow/2}h</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-[9px] font-black text-white uppercase tracking-widest">Nyní</span>
            <div className="w-1 h-1 rounded-full bg-white mt-1" />
          </div>
          <div className="flex flex-col items-center">
            <span className="text-[9px] font-black text-slate-600 uppercase tracking-tighter">Predikce</span>
            <span className="text-[8px] font-bold text-slate-700">+{settings.chartWindow/2}h</span>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-4">
        <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] ml-2">Log Aktivních Dávek</h3>
        {activeDoses.length > 0 ? (
          activeDoses.map(dose => {
            ...
            return (
              <div key={dose.id} className="bg-white/[0.02] backdrop-blur-xl rounded-3xl p-5 border border-white/[0.05] flex items-center justify-between group hover:bg-white/[0.04] transition-all">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-white/[0.03] flex items-center justify-center border border-white/[0.05] group-hover:scale-105 transition-transform">
                    <Activity size={20} style={{ color: substance.color || '#00d1ff' }} />
                  </div>
                  <div>
                    <div className="text-sm font-black text-slate-100">{substance.name}</div>
                ...
                <div className="text-right">
                  <div className="text-xs font-black text-white mb-1.5">{level.toFixed(0)}%</div>
                  <div className="w-20 h-1.5 bg-white/[0.02] rounded-full overflow-hidden border border-white/[0.05]">
                    <div 
          ...
          <div className="p-10 text-center bg-white/[0.01] rounded-[32px] border border-white/5 border-dashed">
            <span className="text-[10px] text-slate-700 font-bold uppercase tracking-[0.3em]">Žádná aktivní data</span>
          </div>
        )}
      </section>
                  ...
                  <div className="p-8">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8">
                      <div className="bg-white/5 rounded-3xl p-5 border border-white/5">
                        ...
                        <div className="text-2xl font-black text-cyan-primary">{level.toFixed(1)}%</div>
                      </div>
                      <div className="bg-white/5 rounded-3xl p-5 border border-white/5">
                        <div className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2">Tolerance</div>
                        <div className="text-2xl font-black text-amber-400">{tolerance.toFixed(0)}%</div>
                      </div>
                    </div>
